...
function mount(fn, parentDomNode, options = {}) {
  let vnode = h(fn);

  // if vnode._t == function
    // then turn into cmp
    // grab a reference to the patch method
    // this will be your redraw
  // else it's a stateless fn
    // maybe do the same as above? idk yet

  // convert renderFn to closureCmp?
  let mountCmp = {
  ...
  options.globalRedraw = true;
  let redraw = _ => render(h(renderFn), parentDomNode, options);
  redraw();
  return redraw;
}
